...
#region Properties
        ...
        {
            // ★追加1: 自分以外のプレイヤー（リモートプレイヤー）の場合はマイク設定が不要なため処理を終了する
            if (Object != null && !Object.HasStateAuthority)
            {
                return;
            }

            // ★追加2: voiceConnection（FusionVoiceClient）が取れていない場合はヌルリフを防ぐため処理を抜ける
            if (this.voiceConnection == null)
            {
                if (this.Logger != null)
                {
                    this.Logger.Log(LogLevel.Warning, "voiceConnection is null. Skipping SetupRecorder.");
                }
                return;
            }
        ...
        public override void Spawned()
        {
            ...
            {
                Debug.LogError(
                    "VoiceNetworkObject requires a FusionVoiceClient on the same GameObject as the NetworkRunner that spawned this object. " +
                    "Add FusionVoiceClient to the NetworkRunner used by Perfect_Online.",
                    this);
                return;
            }
        ...
        {
            if (this.voiceConnection != null && this.RecorderInUse != null)
            {
