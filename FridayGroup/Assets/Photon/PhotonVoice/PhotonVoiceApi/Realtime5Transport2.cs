// -----------------------------------------------------------------------
// <copyright file="Realtime5Transport2.cs" company="Exit Games GmbH">
...
namespace Photon.Voice
{
    ...
    /// <summary>
    /// Variant of Realtime5Transport. Aims to be non-alloc at the cost of breaking compatibility with older clients.
    /// </summary>
    public class Realtime5Transport2 : Realtime5Transport
    {
        public Realtime5Transport2(ILogger logger = null, Client.ConnectionProtocol connectionProtocol = Client.ConnectionProtocol.Udp, bool cppCompatibilityMode = false) : base(logger, connectionProtocol, cppCompatibilityMode)
        {
            this.RealtimePeer.UseByteArraySlicePoolForEvents = true; // incoming byte[] events can be deserialized to a pooled ByteArraySlice
            this.RealtimePeer.ReuseEventInstance = true;             // this won't store references to the event anyways
        }
        ...
        protected override object buildFrameMessage(byte voiceId, ushort evNumber, byte frNumber, ArraySegment<byte> data, FrameFlags flags)
        {
            ...
            Client.ByteArraySlice frameData = this.RealtimePeer.ByteArraySlicePool.Acquire(data.Count + MAX_DATA_OFFSET);
        ...
        protected override void onEventActionVoiceClient(Client.EventData ev)
        {
            if (ev.Code == VoiceEvent.FrameCode)
            {
                ...
                this.onVoiceFrameEvent(ev[(byte)Realtime.ParameterCode.CustomEventContent], REMOTE_VOICE_CHANNEL, ev.Sender, this.LocalPlayer.ActorNumber);
            }
            else
            {
        ...
        internal void onVoiceFrameEvent(object content0, int channelId, int playerId, int localPlayerId)
        {
            ...
            int contentLength;
            int sliceOffset = 0;
            Client.ByteArraySlice slice = content0 as Client.ByteArraySlice;
            if (slice != null)
            {
            ...
            {
                this.logger.Log(Voice.LogLevel.Error, "[PV] onVoiceFrameEvent did not receive data (readable as byte[]) " + content0);
            }
            else
            {
