// -----------------------------------------------------------------------
// <copyright file="Realtime5Transport.cs" company="Exit Games GmbH">
...
namespace Photon.Voice
{
    class VoiceEvent
    {
        ...
        public const byte FrameCode = 203; // Realtime5Transport2 uses separate code for frame event serialized as byte[]
    }

    /// <summary>
    /// Extends RealtimeClient with media streaming functionality.
    /// </summary>
    /// <remarks>
    /// Use your normal Realtime workflow to join a Voice room.
    /// All standard Realtime features are available.
    ...
    public class Realtime5Transport : RealtimeClient, IVoiceTransport, IDisposable
    {
        ...
        /// <summary>The <see cref="VoiceClient"></see> implementation associated with this Realtime5Transport.</summary>
        ...
        /// <summary>
        /// Initializes a new <see cref="Realtime5Transport"/>.
        /// </summary>
        /// <param name="logger">ILogger instance. If null, this instance RealtimeClient.DebugReturn implementation is used.<see cref="ConnectionProtocol"></see></param>
        ...
        public Realtime5Transport(ILogger logger = null, Client.ConnectionProtocol connectionProtocol = Client.ConnectionProtocol.Udp, bool cppCompatibilityMode = false) : base(connectionProtocol)
        {
            if (logger == null)
            {
            ...
            {
                this.RealtimePeer.ChannelCount = 4;
            }
        ...
        RaiseEventArgs buildEvOptFromTargets(bool targetMe, int[] targetPlayers)
        {
            ...
            {
                if (opt.TargetActors != null && opt.TargetActors.Length == 0) // Voice Core does not do such calls but better check again because Realtime sends to all except the local client if the list is empty
                {
        ...
        public void SendVoiceInfo(LocalVoice voice, int channelId, bool targetMe, int[] targetPlayers)
        {
            ...
            var sendOpt = new Client.SendOptions()
            {
                DeliveryMode = Client.DeliveryMode.Reliable,
        ...
        public void SendFrame(ArraySegment<byte> data, FrameFlags flags, ushort evNumber, byte frNumber, byte voiceId, int channelId, SendFrameParams par)
        {
            ...
            var sendOpt = new Client.SendOptions()
            {
                DeliveryMode = ((flags & FrameFlags.Config) != 0) ? Client.DeliveryMode.Reliable : // config frame should be send in sync with voice info
                    cppCompatibilityMode ?
                        par.Reliable ? Client.DeliveryMode.Reliable : Client.DeliveryMode.Unreliable :
                        par.Reliable ? Client.DeliveryMode.ReliableUnsequenced : Client.DeliveryMode.UnreliableUnsequenced,
                Channel = (byte)channelId,
                Encrypt = par.Encrypt,
            };
            ...
            while (this.RealtimePeer.SendOutgoingCommands()) ;
        }

        public string ChannelIdStr(int channelId) { return null; }
        public string PlayerIdStr(int playerId) { return null; }

        protected virtual void onEventActionVoiceClient(Client.EventData ev)
        {
        ...
        /// <summary>
        /// Releases all resources used by the <see cref="Realtime5Transport"/> instance.
        /// </summary>
        public void Dispose()
        {
        ...
        class LBCLogger : ILogger
        {
            ...
            public LogLevel Level
            {
                get
                {
                    if (lbt.LogLevel == Client.LogLevel.Info) return Voice.LogLevel.Info;
                    if (lbt.LogLevel == Client.LogLevel.Warning) return Voice.LogLevel.Warning;
                    if (lbt.LogLevel == Client.LogLevel.Error) return Voice.LogLevel.Error;
                    return Voice.LogLevel.Trace;
                }
            }
            ...
            public void Log(LogLevel level, string fmt, params object[] args)
            {
                if (this.Level >= level)
                {
                    Client.LogLevel debugOut = Client.LogLevel.Debug;
                    if (level == Voice.LogLevel.Info) debugOut = Client.LogLevel.Info;
                    else if (level == Voice.LogLevel.Warning) debugOut = Client.LogLevel.Warning;
                    else if (level == Voice.LogLevel.Error) debugOut = Client.LogLevel.Error;
